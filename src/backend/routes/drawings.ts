...
router.get("/books/:bookId/drawings", (req, res) => {
  ...
  const rows = getDb()
    .prepare(
      hasRange
        ? "SELECT * FROM drawing_overlays WHERE book_id = ? AND overlay_type = 'scribble' AND page_number BETWEEN ? AND ? ORDER BY page_number ASC"
        : "SELECT * FROM drawing_overlays WHERE book_id = ? AND overlay_type = 'scribble' ORDER BY page_number ASC"
    )
...
router.get("/books/:bookId/getting-started/:pageNumber", (req, res) => {
  ...
  const row = getDb()
    ...
    | { id: string; summary_text: string; updated_at: string; llm_model: string | null }
    | undefined;
  if (!row) return res.json({ item: null });
  res.json({ item: { id: row.id, summary_text: row.summary_text, updated_at: row.updated_at, llm_model: row.llm_model } });
});
...
router.post("/books/:bookId/getting-started/:pageNumber", async (req, res, next) => {
  try {
    ...
    const pageContextText = typeof req.body.page_context_text === "string" ? req.body.page_context_text : "";
    if (!screenshotDataUrl) return res.status(400).json({ error: "screenshot_data_url is required." });
    const settings = getAppSettings();
    const { providerId, model } = selectGettingStartedModel(settings);
    ...
    const response = await provider.chat({
      ...
      messages: [
        {
          role: "system",
          content: `You help readers get oriented on one textbook page.
Return JSON only with a single string key named "summary_text".
Write a substantive but concise guide, usually 5-8 bullets or three short sections.
Use the screenshot as the source of truth for layout and equations, and use neighboring text only for context.
Do not include coordinates, overlays, strokes, boxes, or visual annotation data.`
        },
        { role: "user", content: `Create a getting-started guide for page ${pageNumber}.
Cover what to read first, what the main idea is, why the displayed formulas/examples matter, and 2-3 questions the reader should keep in mind.

Current page text (possibly partial OCR/text layer):
${pageText.slice(0, 8000)}

Nearby page text, if available:
${pageContextText.slice(0, 12000)}`, attachments: [{ type: "image", dataUrl: screenshotDataUrl, mimeType: "image/png" }] }
      ]
    }, apiKey);
    ...
    getDb().prepare(`INSERT INTO getting_started_pages (id, book_id, page_number, summary_text, overlay_strokes_json, screenshot_data_url, llm_model, created_at, updated_at)
      ...
      .run(existing?.id ?? id("gstart"), req.params.bookId, pageNumber, summary, json([]), screenshotDataUrl, `${providerId}:${model}`, existing?.created_at ?? now, now);
    res.json({ item: { summary_text: summary } });
  } catch (error) {
    next(error);
  }
});

function selectGettingStartedModel(settings: ReturnType<typeof getAppSettings>) {
  const configuredChoice =
    settings.modelMode === "detailed"
      ? settings.chatModels.pdf_thinking
  ...
  return { providerId, model: normalizeModel(providerId, configuredChoice.model) };
}
...
function extractSummaryField(input: string): string | null {
  const summaryMatch = input.match(/"summary(?:_text)?"\s*:\s*"([\s\S]*?)(?:"\s*,\s*"[^"]+"|"\\n\s*}\s*$|"\s*}\s*$)/i);
